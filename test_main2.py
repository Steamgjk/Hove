...
#test_vgg = VGG_O('VGG19')
#print(test_vgg)
#print("para", test_vgg.parameters())
#test_optimizer = optim.SGD(test_vgg.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)
...
#print("sub_net1 OK", sub_net1.parameters())
...
#print("sub_net2 OK", sub_net2.parameters())
...
#print("main_net OK", main_net.parameters())
summary(sub_net1, (3,32,32))
summary(main_net, (3,32,32))
#替换掉module的grad
f = open("log2.txt", "w") 
print("log OK")
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # 断开之后的requires_grad 必须设置为True，经过验证，
        #调用库函数，如果自己穿进去的input requires_grad=False
        ...
        loss.backward()
        print("Backward 2 ??? ")
        #print(sub_optimizer2.param_groups)
        print("----------------------")
        #print(sub_optimizer1.param_groups)
        sub_optimizer2.step()
        ...
        #print(sub_net1.feature_arr[0].weight.data)
        temp = sub_net1.feature_arr[0].weight.clone()
        fe = sub_optimizer1.param_groups[0]["params"][0]
        print("ID:", id(temp))
        print("ID:", id(fe))
        print("ID:", id(sub_optimizer1.param_groups[0]["params"][0]))
        print(torch.numel(fe.data))
        print(torch.numel(temp.data))
        print("Whe??", fe.equal(temp))
        print("Group Len ", len(sub_optimizer1.param_groups))
        for group in sub_optimizer1.param_groups:
            print("Len:",len(group["params"]))
        #raw_input("-------------Grad-------------------")
        #print(sub_net1.feature_arr[0].weight.grad)
        #raw_input("-------------Origi -------------------")
        #torch.nn.init.constant(sub_net1.feature_arr[0].weight.grad, 10000000)
        torch.nn.init.constant_(fe.grad, 10000000)
        print("Mean:", fe.grad.mean())
        #print(sub_net1.feature_arr[0].weight.grad)
        #raw_input("-------------Grad 100-------------------")
        sub_optimizer1.step()
        raw_input("T stop")
        ...
        #print(sub_net1.feature_arr[0].weight.data)
        temp2 = torch.autograd.Variable(sub_net1.feature_arr[0].weight.data)

        print("Equal?? ", temp.equal(temp2), temp.mean(), fe.mean(), temp2.mean())
        print("Equal2??", fe.equal(temp))


        #for i in range(39):
            #if(hasattr(sub_net1.feature_arr[i], "weight") and (not sub_net1.feature_arr[i].weight is None) ):
                #print(i, "weight num", torch.numel(sub_net1.feature_arr[i].weight.data), "grad num",torch.numel(sub_net1.feature_arr[i].weight.grad) )

...
parser.add_argument('--tokencap', default="32", type=int, help='Master Port')
...
def train_sync_proc_bk(wid):
    ...
    while True:
        ...
        if ts2worker_tensor[0]== NO_AVAILABLE:
            break
        else:
            depth_list.append(int(ts2worker_tensor[1]))
            token_list.append(int(ts2worker_tensor[2]))
    print("len=",len(depth_list))
    print(depth_list)
    print(token_list)
    time_list = []
    while True:
...
def train_sync_proc(wid):
    ...
    NEED_SYNC.zero_()
    print("NEED:",NEED_SYNC, "\t", NEED_SYNC.sum())
    print("CAN:",CAN_SYNC)
    dist.send(tensor = worker2ts_tensor, dst = dst_rank)
    while True:
        #print("RECV...")
        ...
        if ts2worker_tensor[0]== DISTRIBUTE_TOKEN:
            ...
            #print("SYNC_CMD...", NEED_SYNC.sum())
            CAN_SYNC.zero_().add_(1)
            ...
            for j in range(TOKEN_LAYERS):
                if ts2worker_tensor[j+1] == TOKEN_CAPACITY/TOKEN_WEIGHT[j]:
                    CAN_SYNC[j] = 1

...
def profile_tensor_shape(batch_size):
    profile_shape = []
    break_points = []
    test_net = GoogLeNet(sta_lidx = -1, ed_lidx = -1)
    out = fake_input
    cnt = 0
    for layer in test_net.features:
        if isinstance(layer, GoogleNetHookLayer):
            print("GoogleNetHookLayer jump off")
            continue
        in_sz = out.size()
        Cin = in_sz[1]
        Hin = in_sz[2]
        ...
        if isinstance(layer, Inception) and cnt >0 :
        	break_points.append(cnt)
        cnt = cnt + 1
    out = out.view(out.size(0), -1)

    layer = test_net.linear
    out = layer(out)
    shp = out.size()

...
def re_partition():
    ...
    for i in range(worker_num-1):
        ...
        while break_ed_idx < len(break_list):
            break_sta = break_list[break_sta_idx]
            break_ed = break_list[break_ed_idx]
            if sum_flops[break_ed-1]-sum_flops[break_sta]<target_workload:
                break_ed_idx += 1
            else:
                new_partition[i+1]=break_ed
                break_sta_idx = break_ed_idx
                break_ed_idx += 1
                break
    new_partition[worker_num]= -1
    #print(new_partition)
    #input("before return new_partition")
...
def get_cache_tensor(idx):
    ...
    grad_data = torch.zeros(shape_list[idx]["weight_size"],dtype=torch.float)
    tensor_data = torch.stack((weight_data,grad_data),0)
    #print("tensor0 sz ", tensor_data[0].size())
    #print("tensor1 sz ", tensor_data[1].size())
    tensor_data = tensor_data.view(1,-1)
    return tensor_data
def pack_layer_info(idx):
    #print("idx = ",idx, " ",shape_list[idx])
    #print("feature type", type(sub_net.feature_arr[idx]))
    if shape_list[idx]["weight_size"] == 0:
        return None
    weight_tensor = sub_net.feature_arr[idx].weight.data.cpu().clone()
    #print("weight_tensor sz ", weight_tensor.size())
    if sub_net.feature_arr[idx].weight.grad is not None:
        print("pack tensor grad size=",sub_net.feature_arr[idx].weight.grad.size())
        grad_tensor = sub_net.feature_arr[idx].weight.grad.cpu().clone()
    else:
        print("grad None ", idx, " ", shape_list[idx])
        grad_tensor = torch.zeros(weight_tensor.size(),dtype=torch.float)
    tensor_data = torch.stack((weight_tensor,grad_tensor),0)
    tensor_data = tensor_data.view(1,-1)
    '''
    num = sub_net.feature_arr[idx].weight.data.numel()
    weight_tensor = torch.zeros((1,num),dtype=torch.float)
    grad_tensor = sub_net.feature_arr[idx].weight.grad.cpu()
    grad_tensor = grad_tensor.view(1,-1)
    print(weight_tensor.numel()," ", grad_tensor.numel())
...
def unpack_tensor(layer_idx, tensor_data):
    ...
    #print(shape_list[layer_idx])
    #print("weight_size=",weight_size)
    weight_data = tensor_data[0,0:weight_size]
    weight_grad = tensor_data[0,weight_size:]
    weight_shape = shape_list[layer_idx]["weight_shape"]
    #print("layer_idx=",layer_idx, " tensor_data=",tensor_data.size(), " weight_data=",weight_data.size(), " weight_grad=",weight_grad.size())
    #print("weight_data.device=",weight_data.device)
    sub_net.feature_arr[layer_idx].weight.data = weight_data.reshape(weight_shape)
    #sub_net.feature_arr[layer_idx].weight.grad = weight_grad.reshape(weight_shape)
    if sub_net.feature_arr[layer_idx].weight.grad is None:
        print("unpack tensor grad is None")
    else:
        print("unpack_tensor ", sub_net.feature_arr[layer_idx].weight.grad.size())
    #print("2 weight_data.device=",sub_net.feature_arr[layer_idx].weight.data.device)
...
def pipe_train():
    ...
    while iter_n<= iteration_num:
        ...
        if forward_iters == batch_thresh and backward_iters == batch_thresh:
            ...
            if(iter_n == 100):
                exit(0)
            if worker_id == 2 and iter_n==5:
                print("switch to cpu")
                os.environ["CUDA_VISIBLE_DEVICES"]=''
                device = 'cpu'
                sub_net = sub_net.to(device)
                sub_optimizer = optim.SGD(sub_net.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)
            if worker_id == 2 and iter_n ==10:
                print("switch to cuda")
                os.environ["CUDA_VISIBLE_DEVICES"]='1'
                device = 'cuda'
                sub_net = sub_net.to(device)
                sub_optimizer = optim.SGD(sub_net.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)
            if worker_id == 3 and iter_n == 11:
                print("switch to cpu")
                os.environ["CUDA_VISIBLE_DEVICES"]=''
                device = 'cpu'
                sub_net = sub_net.to(device)

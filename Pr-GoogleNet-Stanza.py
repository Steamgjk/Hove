	...
	if is_fc(wid):
			...
			for i in range(subitern):
				...
				for conv_id in range(conv_n):
					if conv_id % fc_n ==  wid - conv_n:
						input_tensor = torch.zeros([args.subbs,1024,1,1])
				...
				back_ctx = GoogleNetHookFunc.backward_ctx
		...
		for j in range(itern):
			...
			for i in range(subitern):
				...
				#print("tosend_data sz=", tosend_data.size())
